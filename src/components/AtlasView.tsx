...
export const AtlasView: React.FC<AtlasViewProps> = ({ visitedCountryIds }) => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col gap-4 sm:gap-6">
      ...
      <div className="bg-slate-900/90 backdrop-blur-md border border-sky-800/40 rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 shadow-xl flex flex-col md:flex-row items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-sky-700/50 rounded-xl sm:rounded-2xl pl-10 pr-4 py-2 sm:py-2.5 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-sky-400 transition-colors"
          />

          {filteredCountries.length > 0 && (
            <div className="absolute left-0 right-0 top-full mt-1.5 bg-slate-900 border border-sky-500/40 rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden z-40">
              {filteredCountries.map((c) => (
                <button
                  key={c.id}
                  onClick={() => handleSelect(c)}
                  className="w-full px-4 py-2.5 text-left text-xs sm:text-sm hover:bg-sky-500/20 text-white flex items-center justify-between border-b border-white/5 last:border-0 cursor-pointer"
                >
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto w-full md:w-auto pb-1 md:pb-0 scrollbar-none">
          ...
          {[
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-xl bg-slate-800 hover:bg-slate-700 border border-white/10 text-[11px] sm:text-xs font-bold text-slate-200 whitespace-nowrap transition-all cursor-pointer active:scale-95 shrink-0"
            >
              {item.label}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="bg-gradient-to-b from-slate-900 to-slate-950 border border-sky-500/30 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-2xl flex flex-col justify-between">
          <div>
